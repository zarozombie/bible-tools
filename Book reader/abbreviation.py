# abbreviation

def shorthand(short):
    if short == "isa" or short == "isaiah":
        return"Isaiah"
    if short == "gen" or short == "genesis":
        return"Genesis"
    elif short == "luk" or short == "luke":
        return "Luke"
    if short == "mat" or short == "matt" or short == "matthew":
        return "Matthew"
    if short == "1chr" or short == "1 chronicles":
        return"1 Chronicles"
    if short == "1cor" or short == "1 co" or short == "1 corinthians" :
        return"1 Corinthians"
    if short == "1 joh" or short == "1 john":
        return "1 John"
    if short == "1ki" or short == "1 kings":
        return "1 Kings"
    if short == "1 pe" or short == "1pe" or short == "1 peter":
        return "1 Peter"
    if short == "1 samuel":
        return"1 Samuel"
    if short == "1 thessalonians":
        return "1 Thessalonians"
    if short == "1 timothy":
        return "1 Timothy"
    if short == "2 chronicles":
        return"2 Chronicles"
    if short == "2 corinthians" or short == "2co":
        return"2 Corinthians"
    if short == "2 john":
        return "2 John"
    if short == "2 kings":
        return "2 Kings"
    if short == "2 peter":
        return "2 Peter"
    if short == "2 sam" or short == "2 samuel":
        return "2 Samuel"        
    if short == "2 thessalonians":
        return "2 Thessalonians"
    if short == "2 ti" or short == "2 timothy":
        return "2 Timothy"
    if short == "3 john":
        return "3 John"
    if short == "act" or short == "acts":
        return "Acts"        
    if short == "amos":
        return "Amos"
    if short == "colossians":
        return "Colossians"
    if short == "dan" or short == "daniel":
        return "Daniel"
    if short == "deuteronomy":
        return "Deuteronomy"
    if short == "ecclesiastes":
        return "Ecclesiastes"
    if short == "eph" or short == "ephesian":
        return "Ephesians"
    if short == "esther":
        return "Esther"
    if short == "exo" or short == "exodus":
        return "Exodus"
    if short == "ezekiel":
        return "Ezekiel"
    if short == "ezra":
        return "Ezra"
    if short == "galatian":
        return "Galatians"
    if short == "habakkuk":
        return "Habakkuk"
    if short == "haggai":
        return "Haggai"
    if short == "hebrews":
        return "Hebrews"
    if short == "hos" or short == "hosea":
        return "Hosea"
    if short == "james":
        return "James"
    if short == "jer" or short == "jeremiah":
        return "Jeremiah"
    if short == "job":
        return "Job"
    if short == "joe" or short == "joel":
        return "Joel"
    if short == "jon" or short == "john":
        return "John"
    if short == "joh" or short == "jonah":
        return "Jonah"
    if short == "jos" or short == "joshua":
        return "Joshua"
    if short == "jude":
        return "Jude"
    if short == "judges":
        return "Judges"
    if short == "lamentations":
        return "Lamentations"
    if short == "lev" or short == "leviticus":
        return "Leviticus"
    if short == "malachi":
        return "Malachi"
    if short == "mark":
        return "Mark"
    if short == "micah":
        return "Micah"
    if short == "nahum":
        return "Nahum"
    if short == "nehemiah":
        return "Nehemiah"
    if short == "numbers" or short == "num":
        return "Numbers"
    if short == "obadiah":
        return "Obadiah"
    if short == "philemon":
        return "Philemon"
    if short == "php" or short == "philippians":
        return "Philippians"
    if short == "proverbs":
        return "Proverbs"
    if short == "pas" or short == "paslms":
        return "Psalms"
    if short == "rev" or short == "revelation":
        return "Revelation"
    if short == "romans":
        return "Romans"
    if short == "ruth":
        return "Ruth"    
    if short == "sirach" or short == "sir":
        return "Sirach"
    if short == "song" or short == "song of solomon":
        return "Song of Solomon"
    if short == "tit" or short == "titus":
        return "Titus"
    if short == "tob" or short == "tobit": 
        return "Tobit"
    if short == "zechariah":
        return "Zechariah"
    if short == "zephaniah":
        return "Zephaniah"
    else:
        return "book or abrev not found!"


# x = "zephaniah"

# ans = shorthand(x.lower())
# ans = shorthand("rev")
# print(ans)
# print("gen")
